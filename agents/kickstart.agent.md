...
Follow `/kickstart-discover`. Use `search` and `search/codebase` to auto-detect language, framework, ports, deps, Dockerfile, CI/CD before asking, and to map each deployable service's build context, entry-point file, and existing Dockerfile path (never assume a flat repo). Collect remaining details via `vscode/askQuestions`. Exit when you have enough to propose architecture.
