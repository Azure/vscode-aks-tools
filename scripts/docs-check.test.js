...
describe("withoutUrls", () => {
    it("blanks a URL so its host is not read as an identifier", () => {
        const out = withoutUrls("See https://dev.azure.com/foo for details");
        assert.ok(!out.includes("dev.azure.com"));
    });

    it("blanks a link target but keeps the text", () => {
        const out = withoutUrls("[the docs](https://learn.microsoft.com/azure.stuff)");
        assert.ok(out.includes("the docs"));
        assert.ok(!out.includes("learn.microsoft.com"));
    });
