...
export async function runClusterProvisioning(
    ...
    if (!(await probeRoleAssignmentAccess())) {
        const accessStage = reporter.stage("roleAccess", l10n.t("Role assignment access"));
        const prompt = await buildProvisioningAccessPrompt(authClient, selections, runId, channel);
        const granted = await accessStage.run(
        ...
        accessStage.succeed(l10n.t("You can now assign roles in {0}.", selections.resourceGroupName));
    }
    ...
    try {
        ...
        result.clusterPortalUrl = await clusterStage.run(
            ...
            () => deployAutomaticCluster(sessionProvider, selections, kubernetesVersion, identity, true),
        );
...
async function runDeploymentVerificationStage(
    ...
    const stage = reporter.stage("verify", l10n.t("Verify registry pull access"));
    const probeResult = await stage.run(l10n.t("Checking the cluster can pull from the registry"), () =>
