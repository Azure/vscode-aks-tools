...
export async function draftDeployment(): Promise<void> {
    ...
    if (failed(azureAccountApi)) {
        window.showErrorMessage(azureAccountApi.error);
        return;
    }

    const { workspaceFolder, extension, draftBinaryPath } = commonDependencies;

    // TODO: We should switch to using @microsoft/vscode-azext-azureauth for authentication.
    // The recommended usage pattern for this is something like:
    ...
    // }
    // Question: should we be signing in at this point with the Azure Account extension?
    // Answer: No, we should not be signing in at this point. The Azure Account extension
    //         is responsible for signing in and we should not be duplicating that work.
