...
async function getDeploymentFiles(
    ...
    const command = `draft create --language ${language} --deployment-only --deploy-type ${deploymentType} ${variableArgs} --destination ${destDir} --dry-run --silent --skip-file-detection`;
