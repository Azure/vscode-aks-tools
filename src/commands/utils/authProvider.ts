...
export async function getAksAadAccessToken(
    environment: Environment,
    serverId: string,
    tenantId: string,
    refreshToken: string,
): Promise<Errorable<AuthenticationResult>> {
    ...
    try {
        ...
        return {
            succeeded: false,
            error: `Failed to acquire AAD token for server ${serverId} from refresh token:\n${e}`,
        };
    }
