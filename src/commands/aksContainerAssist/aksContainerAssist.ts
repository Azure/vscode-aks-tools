...
import { ContainerAssistAction, ContainerAssistQuickPickItem, DeploymentArtifacts } from "./types";
...
export async function containerizeApp(context: IActionContext, target: unknown): Promise<void> {
    await runContainerAssist(context, target, [
        ContainerAssistAction.GenerateDockerfile,
        ContainerAssistAction.GenerateManifests,
    ]);
}

export async function containerizeAppFromTree(context: IActionContext, target: unknown): Promise<void> {
    await runContainerAssistFromTree(context, target, [
        ContainerAssistAction.GenerateDockerfile,
        ContainerAssistAction.GenerateManifests,
    ]);
}
 ...
 * artifact generation, and post-generation options.
 *
 * The user may independently choose to generate a Dockerfile, Kubernetes manifests, and/or a
 * GitHub workflow:
 * - A Dockerfile alone needs no Azure context.
 * - Manifests need a namespace and an image source (ACR or an existing image reference).
 * - A workflow additionally requires an ACR and the Dockerfile + manifests to exist.
 */
async function executeContainerAssistActions(
    ...
    const wantDockerfile = selectedActions.includes(ContainerAssistAction.GenerateDockerfile);
    const wantManifests = selectedActions.includes(ContainerAssistAction.GenerateManifests);
    const wantWorkflow = selectedActions.includes(ContainerAssistAction.GenerateWorkflow);
    const wantDeploymentArtifacts = wantDockerfile || wantManifests;

    // The Dockerfile and manifests are generated by the language model.
    if (wantDeploymentArtifacts && !(await ensureLanguageModelSelected(containerAssistService))) {
        return;
    }

    // Azure context is required for manifests (namespace + image) and for the workflow
    // (cluster + ACR + workflow name). Generating only a Dockerfile needs none of it.
    let azureContext: AzureContext | undefined;
    if (wantManifests || wantWorkflow) {
        azureContext = await azureContextProvider(wantWorkflow);
        if (!azureContext) return;
    }

    // When a workflow is generated alongside deployment artifacts, defer the post-generation
    // options so every generated file is presented together at the end.
    const deferPostGeneration = wantWorkflow && wantDeploymentArtifacts;

    let deploymentResult: ActionResult | undefined;
    if (wantDeploymentArtifacts) {
        deploymentResult = await generateDeploymentFiles(
            containerAssistService,
            workspaceFolder,
            projectRoot,
            { dockerfile: wantDockerfile, manifests: wantManifests },
            deferPostGeneration,
            azureContext,
        );
    }
    ...
    if (!wantWorkflow) {
        // Deployment-only: post-generation options were already shown by generateDeploymentFiles.
        return;
    }

    // azureContext is guaranteed here because wantWorkflow implies it was collected above.
    if (!azureContext) return;

    if (!wantDeploymentArtifacts) {
        // Workflow-only: let the workflow generator detect/prompt for the existing Dockerfile and manifests.
        await generateWorkflowFile({ workspaceFolder, projectRoot, azureContext, hasBothActions: false });
        return;
    }

    await generateWorkflowForDeploymentArtifacts(
        containerAssistService,
        workspaceFolder,
        projectRoot,
        azureContext,
...
/**
 * Ensures a language model is available and selected before generating LLM-authored artifacts.
 * Surfaces any error to the user. Returns false when the caller should abort.
 */
async function ensureLanguageModelSelected(containerAssistService: ContainerAssistService): Promise<boolean> {
...
/**
 * Generates a workflow that builds and deploys freshly generated deployment artifacts, then
 * presents all generated files together. The workflow depends on a Dockerfile + manifests, so
 * this verifies both exist on disk before proceeding.
 */
async function generateWorkflowForDeploymentArtifacts(
    containerAssistService: ContainerAssistService,
    workspaceFolder: vscode.WorkspaceFolder,
    projectRoot: string,
    azureContext: AzureContext,
    deploymentResult: ActionResult | undefined,
): Promise<void> {
...
interface ActionResult {
    ...
    workflowPath?: string;
    /** Absolute paths to any generated Kubernetes manifests. */
...
/**
 * Builds a per-module image repository resolver: the custom image reference verbatim, or
 * `{acrName}.azurecr.io/{moduleName}` for ACR. Returns undefined when no image source is set.
 */
function buildImageRepositoryResolver(
    azureContext: AzureContext,
): ((moduleName: string) => string | undefined) | undefined {
    if (azureContext.customImage) {
        const image = azureContext.customImage;
        return () => image;
    }

    if (azureContext.acrName) {
        const acrLoginServer = `${azureContext.acrName}.azurecr.io`;
        return (moduleName: string) => `${acrLoginServer}/${moduleName}`;
    }
...
async function generateDeploymentFiles(
    ...
    deferPostGeneration: boolean,
    azureContext?: AzureContext,
): Promise<ActionResult | undefined> {
    const displayName = path.basename(targetPath);

    const imageRepositoryFor = azureContext ? buildImageRepositoryResolver(azureContext) : undefined;
    ...
    // Defer post-generation options when a workflow is also being generated.
    if (deferPostGeneration) {
        return {
