import { combine, Errorable, failed, getErrorMessage, succeeded } from '../../utils/errorable';
...
import { AzureAccountExtensionApi, AzureSession } from '../../utils/azureAccount';
...
export async function getServicePrincipalAccess(apiAzureAccount: AzureAccountExtensionApi, appId: string, secret: string): Promise<Errorable<ServicePrincipalAccess>> {
    ...
    const promiseResults = await Promise.all(apiAzureAccount.filters.map(f => getSubscriptionAccess(spInfo.result.credential, f.subscription, spInfo.result)));
    const ownershipResults = combine(promiseResults);
    if (failed(ownershipResults)) {
        return ownershipResults;
    }

    const subscriptions = ownershipResults.result.filter(r => r.hasRoleAssignment).map(r => ({
...
async function getServicePrincipalInfo(sessions: AzureSession[], appId: string, appSecret: string): Promise<Errorable<ServicePrincipalInfo>> {
    const spInfoResults = await Promise.all(sessions.map(s => getServicePrincipalInfoForSession(s, appId, appSecret)));
...
async function getServicePrincipalInfoForSession(session: AzureSession, appId: string, appSecret: string): Promise<Errorable<ServicePrincipalInfo>> {
    ...
    if (!spSearchResults.value || spSearchResults.value.length !== 1) {
        return { succeeded: false, error: `Expected service principal search result to contain value with one item. Actual result: ${JSON.stringify(spSearchResults)}` };
    }
...
async function getSubscriptionAccess(credential: TokenCredential, subscription: Subscription, spInfo: ServicePrincipalInfo): Promise<Errorable<SubscriptionAccessResult>> {
