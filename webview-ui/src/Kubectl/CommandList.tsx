...
export function CommandList(props: CommandListProps) {
            ...
            <li>
                ...
                <ul className={styles.commandList}>
                    {commands.map(command => (
                        <li key={command.name} className={command.command === props.selectedCommand ? styles.selected : ''} onClick={() => props.onSelectionChanged(command)}>
                            <div>
                                <h4>{command.name}</h4>
                                <pre>{command.command}</pre>
                            </div>
                            {command.category === CommandCategory.Custom && (
                                <FontAwesomeIcon className={styles.commandDelete} style={{cursor: "pointer"}} icon={faTrashCan} onClick={e => handleCommandDelete(e, command.name)} />
                            )}
                        </li>
                    ))}
                </ul>
    ...
    return (
        <ul id={props.id} className={props.className ? `${props.className} ${styles.commandCategoryList}` : styles.commandCategoryList}>
