...
export function toSortString(sortSpecifiers: SortSpecifier<string>[]): string {
    return sortSpecifiers
        .map((s) => `${s.direction === SortDirection.Descending ? "-" : ""}${s.property.identifier}`)
        .join(",");
}
...
export function isDerivedProperty<TKey extends string, TDerivedKey extends string>(
    gadgetProperty: ItemProperty<TKey | TDerivedKey>,
): gadgetProperty is DerivedItemProperty<TKey, TDerivedKey> {
...
export function getDerivedProperty<TKey extends string, TDerivedKey extends string>(
    name: string,
    key: TDerivedKey,
    valueGetter: (item: DataItem<TKey>) => unknown,
    valueType?: ValueType,
): DerivedItemProperty<TKey, TDerivedKey> {
    return { name, identifier: key, key, valueGetter, valueType };
}

export function findProperty<TKey extends string>(
    properties: ItemProperty<TKey>[],
    key: TKey,
): ItemProperty<TKey> | null {
...
export function toProperties<TKey extends string>(
    allProperties: ItemProperty<TKey>[],
    keys: TKey[],
): ItemProperty<TKey>[] {
...
export function getSortSpecifiers<TKey extends string>(
    properties: ItemProperty<TKey>[],
    specifiers: { key: TKey; direction: SortDirection }[],
): SortSpecifier<TKey>[] {
    return specifiers
        .map((s) => ({
