...
export function AzureServiceOperator(initialState: InitialState) {
    ...
    useEffect(() => {
        ...
        if (
            state.installCertManagerStep.status === InstallStepStatus.Succeeded &&
            state.waitForCertManagerStep.status === InstallStepStatus.NotStarted
        ) {
        ...
        if (
            state.waitForCertManagerStep.status === InstallStepStatus.Succeeded &&
            state.installOperatorStep.status === InstallStepStatus.NotStarted
        ) {
        ...
        if (
            state.installOperatorStep.status === InstallStepStatus.Succeeded &&
            state.installOperatorSettingsStep.status === InstallStepStatus.NotStarted
        ) {
        ...
        if (
            state.installOperatorSettingsStep.status === InstallStepStatus.Succeeded &&
            state.waitForControllerManagerStep.status === InstallStepStatus.NotStarted
        ) {
