...
function getPanelClassNames(status: Status) {
    switch (status) {
        case Status.Success: return `${styles.detectorPanel} ${styles.success}`;
        case Status.Warning: return `${styles.detectorPanel} ${styles.warning}`;
        default: return `${styles.detectorPanel} ${styles.error}`;
    }
}
