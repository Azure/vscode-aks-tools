...
export function getASOScenarios() {
    ...
    function getMessageHandler(
        ...
        return {
            ...
            installOperatorSettingsRequest: (args) =>
                handleInstallOperatorSettingsRequest(
                    withErrors && sometimes(),
                    args.tenantId,
                    args.subscriptionId,
                    args.appId,
                    args.appSecret,
            ...
            waitForControllerManagerRequest: () =>
                handleWaitForControllerManagerRequest(withErrors && sometimes(), webview),
        };
    }
    ...
    function getInstallStepResult(hasError: boolean, stepName: string, commands: string[]): InstallStepResult {
        if (hasError) {
            return {
                ...
                commandResults: [
                    ...commands.slice(0, -1).map((c) => getCommandResult(false, c)),
                    getCommandResult(true, commands[commands.length - 1]),
                ],
            };
        }
        return {
    ...
    async function handleWaitForCertManagerRequest(hasError: boolean, webview: MessageSink<ToWebViewMsgDef>) {
        ...
        const commands = ["cert-manager", "cert-manager-cainjector", "cert-manager-webhook"].map(
            (c) => `kubectl check deployment ${c}`,
        );
    ...
    async function handleInstallOperatorSettingsRequest(
        hasError: boolean,
        tenantId: string,
        ...
        await new Promise((resolve) => setTimeout(resolve, 1000));
        const commands = [
            `kubectl create secret [tenantId=${tenantId}, subId=${subscriptionId}, appId=${appId}, secret=${appSecret}, cloud=${cloudName}]`,
        ];
        webview.postInstallOperatorSettingsResponse(
            getInstallStepResult(hasError, "install settings secret", commands),
        );
    }
    ...
    async function handleWaitForControllerManagerRequest(hasError: boolean, webview: MessageSink<ToWebViewMsgDef>) {
        ...
        webview.postWaitForControllerManagerResponse(
            getInstallStepResult(hasError, "wait for controller manager", commands),
        );
    }
    ...
    return [
        Scenario.create(
            "aso",
            "successful",
            () => <AzureServiceOperator {...initialState} />,
