...
export function getDraftDeploymentScenarios() {
    ...
    function getMessageHandler(
        ...
        async function handleGetResourceGroupsRequest(subscriptionKey: SubscriptionKey) {
            await delay(2000);
            const subData = allSubscriptionData.find((d) => d.subscription.id === subscriptionKey.subscriptionId);
            const groupData = subData?.resourceGroups || [];
            webview.postGetResourceGroupsResponse({
                ...subscriptionKey,
                groups: groupData.map((g) => g.group),
            });
        }

        async function handleGetAcrsRequest(resourceGroupKey: ResourceGroupKey) {
            await delay(2000);
            const subData = allSubscriptionData.find((d) => d.subscription.id === resourceGroupKey.subscriptionId);
            const groupData = subData?.resourceGroups.find((g) => g.group === resourceGroupKey.resourceGroup);
            const acrs = groupData?.acrs || [];
            webview.postGetAcrsResponse({
                ...resourceGroupKey,
                acrNames: acrs.map((a) => a.acr),
            });
        }
        ...
        async function handleGetClustersRequest(resourceGroupKey: ResourceGroupKey) {
            await delay(2000);
            const subData = allSubscriptionData.find((d) => d.subscription.id === resourceGroupKey.subscriptionId);
            const groupData = subData?.resourceGroups.find((g) => g.group === resourceGroupKey.resourceGroup);
            const clusters = groupData?.clusters || [];
            webview.postGetClustersResponse({
                ...resourceGroupKey,
                clusterNames: clusters.map((c) => c.cluster),
            });
        }
