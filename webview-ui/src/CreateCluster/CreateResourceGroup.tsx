...
export function CreateResourceGroupDialog (props: CreateResourceGroupDialogProps) {
    ...
    const handleNameChange = createHandler<string, ChangeEvent, HTMLInputElement>(
        ...
        elem => elem.validity.patternMismatch ? "Resource Group name contains invalid characters."
            : elem.validity.tooShort ? "Resource Group name must be at least 1 character long."
            : elem.validity.tooLong ? "Resource Group name must be at most 90 characters long."
            : elem.validity.valueMissing ? "Resource Group name is required."
    ...
    <Dialog isShown={props.isShown} onCancel={() => props.onCancel()}>
        ...
        <form className={styles.createForm} onSubmit={handleSubmit}>
            <div className={styles.inputContainer}>
                ...
                <label htmlFor="rg-name-input" className={styles.label}>Name</label>
                <VSCodeTextField
                    id="rg-name-input"
                    value={name.value || ''}
                ...
                <label htmlFor="location-dropdown" className={styles.label}>Location</label>
                <VSCodeDropdown
                ...
                >
                    ...
                    {props.locations.map(location => (
                        <VSCodeOption key={location} value={location}>{location}</VSCodeOption>
                    ))}
                </VSCodeDropdown>
                {
