...
Every action in these templates is pinned to a commit SHA rather than a tag. The tables
below name the actions without versions, because those move; for the version each one is
currently pinned to, see [Pinned versions](../reference/pinned-versions.md), which is
generated from the templates themselves.
